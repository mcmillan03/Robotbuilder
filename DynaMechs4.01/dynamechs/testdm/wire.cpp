...
dmRevoluteLink  *roll_link;
dmPrismaticLink *translate_link;
Float  desired_roll, desired_trans;

//----------------------------------------------------------------------------
void initControl(dmArticulation *articulation)
{
   Float joint_vel;

   int num_links = articulation->getNumLinks();
   if (num_links < 2)
   {
      cerr << "Error: not enough links: " << num_links << endl;
      exit(3);
   }

   dmObject* obj = dmuFindObject("translation", articulation);
   translate_link = dynamic_cast<dmPrismaticLink*>(obj);
   if (translate_link == NULL)
   {
      cerr << "Error: no translation link found" << endl;
      exit(3);
   }
   translate_link->getState(&desired_trans, &joint_vel);

   obj = dmuFindObject("roll", articulation);
   roll_link = dynamic_cast<dmRevoluteLink*>(obj);
   if (roll_link == NULL)
   {
      cerr << "Error: no roll link found" << endl;
      exit(3);
   }
   roll_link->getState(&desired_roll, &joint_vel);
}
...
{
   static Float joint_input;
   static Float joint_pos;
   static Float joint_vel;
   static Float delta_pos;

   delta_pos = 22.5*sin(0.4*time)*sin(0.4*time);
   translate_link->getState(&joint_pos, &joint_vel);
   joint_input = 75.0*(desired_trans +
                       delta_pos - joint_pos) - 2.0*joint_vel;
   translate_link->setJointInput(&joint_input);


   delta_pos = 3.0*sin(2.0*time);
   roll_link->getState(&joint_pos, &joint_vel);
   joint_input = 75.0*(desired_roll +
                       delta_pos - joint_pos)  - 2.0*joint_vel;
   roll_link->setJointInput(&joint_input);
}
