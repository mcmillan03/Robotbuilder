 ...
 *  Created: 1999
 *  Summary: Class definitions for prismatic secondary joints.
 *****************************************************************************/

#ifndef _DM_SECONDARY_PRISMATIC_JOINT_HPP
#define _DM_SECONDARY_PRISMATIC_JOINT_HPP
...
The {\tt dmSecondaryPrismaticJoint} class can model loop-closing
joints with one translational degree of freedom.  The class is derived
from the {\tt dmSecondaryJoint} class.  The state of the joint is
computed based on a six-degree of freedom joint from the outboard
coordinate system of link A to that of link B.  The joint consists of
...
free joint variable is the $\hat{z}$ translation, $d_z$, while the
vector of constrained joint variables is given by
$[\alpha~\beta~\gamma~d_x~d_y]^T$.  Note that the simulation algorithm
...
class for the specific case of prismatic secondary joints.
...
class DM_DLL_API dmSecondaryPrismaticJoint :  public dmSecondaryJoint
{
public:
   enum {NUM_DOFS = 1};
...
private:   // not implemented
   dmSecondaryPrismaticJoint(const dmSecondaryPrismaticJoint &);
   dmSecondaryPrismaticJoint &operator=(const dmSecondaryPrismaticJoint &);
