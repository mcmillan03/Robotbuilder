...
void dmArticulation::draw() const
{
   ...
   if (len > 1.0e-6)
   {
      float angle = 2.0*atan2(len, (float)m_quat_ICS[3]);
...
void dmQuaternionLink::draw() const
{
   ...
   if (len > 1.0e-6)
   {
      float angle = 2.0*atan2(len,(float) m_q[3]);
...
void dmMobileBaseLink::draw() const
{
   ...
   if (len > 1.0e-6)
   {
      float angle = 2.0*atan2(len,(float) m_quat[3]);
...
void dmTreadmill::draw() const
{
   ...
   float dxl = m_half_width*m_treadmill_y[0];
   float dyl = m_half_width*m_treadmill_y[1];
   float dzl = m_half_width*m_treadmill_y[2];
