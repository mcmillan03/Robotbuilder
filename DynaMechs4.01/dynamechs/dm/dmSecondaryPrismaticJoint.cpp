 ...
 *****************************************************************************
 *     File: dmSecondaryPrismaticJoint.cpp
 *   Author: Duane Marhefka
 *  Created: 1999
 *  Summary: Class declaration for prismatic secondary joints.
 *****************************************************************************/

#include <dmSecondaryPrismaticJoint.hpp>

//============================================================================
// class dmSecondaryPrismaticJoint : public dmSecondaryJoint
//============================================================================


//----------------------------------------------------------------------------
//    Summary: constructor for dmSecondaryPrismaticJoint class
// Parameters: none
//    Returns: none
//----------------------------------------------------------------------------
dmSecondaryPrismaticJoint::dmSecondaryPrismaticJoint()
      : dmSecondaryJoint(),
        m_pos_spring(0.0),
        m_pos_damper(0.0),
        m_euler_spring(0.0),
...
//    Returns: none
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::setConstraintParams(Float linear_spring,
                                                    Float linear_damper,
...
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::getFreeState(Float q[], Float qd[]) const
{
   q[0] = m_d[2];
   qd[0] = m_d_dot[2];
}
...
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::getConstrainedState(Float q[],
                                                    Float qd[]) const
{
   q[0] = m_euler[0];
   q[1] = m_euler[1];
...
//----------------------------------------------------------------------------
//    Summary: Compute the state (free and constrained) of the prismatic
//             secondary joint.
// Parameters: none
//    Returns: none
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::computeState()
{
#ifdef DEBUG
   cout << "dmSecondaryPrismaticJoint::computeState() enter\n" << flush;
#endif
...
//    Returns: none
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::computeEtas()
{
#ifdef DEBUG
   cout << "dmSecondaryPrismaticJoint::computeEtas() enter\n" << flush;
#endif
...
//    Returns: none
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::computeAppliedForce()
{
   m_k_f_k[0] = m_k_f_k[1] = m_k_f_k[2] = 0.0;

   m_k_f_k[3] = m_oa_R_k[2][0]*(m_joint_input - m_joint_friction*m_d_dot[2]);
   m_k_f_k[4] = m_oa_R_k[2][1]*(m_joint_input - m_joint_friction*m_d_dot[2]);
   m_k_f_k[5] = m_oa_R_k[2][2]*(m_joint_input - m_joint_friction*m_d_dot[2]);
}
...
//    Returns: none
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::applyPenaltyForce()
{
#ifdef DEBUG
   cerr << "dmSecondaryPrismaticJoint::applyPenaltyForce()" << flush << endl;
#endif
   ...
   gimbal_trq[2] = -m_euler[2]*m_euler_spring - m_euler_dot[2]*m_euler_damper;
   ...
   m_k_f_k[0] +=  m_c2/m_c1*gimbal_trq[0] +
      m_s2*gimbal_trq[1] - m_c2*m_t1*gimbal_trq[2];
   m_k_f_k[1] += -m_s2/m_c1*gimbal_trq[0] +
      m_c2*gimbal_trq[1] + m_s2*m_t1*gimbal_trq[2];
   m_k_f_k[2] += gimbal_trq[2];
...
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::computeStabilizationForce(Float force[])
{
   // Gimbal torques.
   ...
   force[2] = -m_euler[2]*m_euler_spring - m_euler_dot[2]*m_euler_damper;

   // Position forces.
   force[3] = -m_d[0]*m_pos_spring - m_d_dot[0]*m_pos_damper;
   force[4] = -m_d[1]*m_pos_spring - m_d_dot[1]*m_pos_damper;
}
...
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::initXik(Float **Xik,
                                        int link_index,
                                        int root_index) const
{
   ...
   {
      // Compute Xik(1:3,1:3)
      Float c2 = cos(m_euler[2]);  // HACK - can I use the member vars?
      Float s2 = sin(m_euler[2]);
      Float c1 = cos(m_euler[1]);
      Float t1 = tan(m_euler[1]);
      for (i = 0; i < 3; i++)
      {
         Xik[i][0] = -( m_a_R_k[i][0]*c2/c1 - m_a_R_k[i][1]*s2/c1 + 0);
         Xik[i][1] = -( m_a_R_k[i][0]*s2    + m_a_R_k[i][1]*c2    + 0);
         Xik[i][2] = -(-m_a_R_k[i][0]*c2*t1 + m_a_R_k[i][1]*s2*t1 +
                       m_a_R_k[i][2]);
      }

      // Compute Xik(4:6,1:3)
      Xik[3][0] = Xik[3][1] = Xik[3][2] = 0.0;
      Xik[4][0] = Xik[4][1] = Xik[4][2] = 0.0;
      Xik[5][0] = Xik[5][1] = Xik[5][2] = 0.0;

      // Compute Xik(4:6,4:5)
      for (i = 0; i < 3; i++)
         for (j = 0; j < 2; j++)
            Xik[3+i][3+j] =  -m_a_R_oa[i][j];
   ...
   else if (link_index == m_link_B_index)  // Xik = X_p2(k)k
   {
      ...
      for (i = 0; i < 3; i++)
      {
         Xik[i][0] =  m_b_R_k[i][0]*c2/c1 - m_b_R_k[i][1]*s2/c1 + 0;
         Xik[i][1] =  m_b_R_k[i][0]*s2    + m_b_R_k[i][1]*c2    + 0;
         Xik[i][2] = -m_b_R_k[i][0]*c2*t1 + m_b_R_k[i][1]*s2*t1 +m_b_R_k[i][2];
      }
      ...
      for (i = 0; i < 3; i++)
         for (j = 0; j < 2; j++)
            Xik[3+i][3+j] =  (m_b_R_k[i][0]*m_oa_R_k[j][0] +
...
//----------------------------------------------------------------------------
void dmSecondaryPrismaticJoint::getZeta(Float *zeta) const
{
   SpatialVector eta;
   ...
   zeta[2] = -m_c2*m_t1*eta[0] + m_s2*m_t1*eta[1] + eta[2];

   zeta[3] = m_oa_R_k[0][0]*eta[3]+m_oa_R_k[0][1]*eta[4]+m_oa_R_k[0][2]*eta[5];
   zeta[4] = m_oa_R_k[1][0]*eta[3]+m_oa_R_k[1][1]*eta[4]+m_oa_R_k[1][2]*eta[5];
   ...
   if (m_stabilization == BAUMGARTE)
   {
      ...
      zeta[2] += -m_euler[2]*m_euler_spring - m_euler_dot[2]*m_euler_damper;
      zeta[3] += -m_d[0]*m_pos_spring - m_d_dot[0]*m_pos_damper;
      zeta[4] += -m_d[1]*m_pos_spring - m_d_dot[1]*m_pos_damper;
   }
}
