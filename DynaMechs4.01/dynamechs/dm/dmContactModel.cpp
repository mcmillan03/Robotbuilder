                                      ...
                                      CartesianVector *contact_pos)
{
   ...
   if (num_contact_points)
   {
      ...
      m_initial_contact_pos_stored = new CartesianVector[m_num_contact_points];
		...
		m_point_IDs = new int [m_num_contact_points];
		m_point_IDs_stored = new int [m_num_contact_points];
...
void dmContactModel::pushState()
{
   for (unsigned int i = 0; i < m_num_contact_points; i++)
   {
      ...
      m_initial_contact_pos_stored[i][2] = m_initial_contact_pos[i][2];
		...
		m_point_IDs_stored[i] = m_point_IDs[i];
   }
}
...
void dmContactModel::popState()
{
   for (unsigned int i = 0; i < m_num_contact_points; i++)
   {
      ...
      m_initial_contact_pos[i][2] = m_initial_contact_pos_stored[i][2];
		...
		m_point_IDs[i] = m_point_IDs_stored[i];
   }
}
                                  ...
                                  SpatialVector f_contact)
{
   ...
   Float ground_elevation;
	...
	// typeid not really a good solution - but can't do this during construction because
	// the environment might not exist yet (depends on the order the user builds the 
	// environment and articulation).
	bool treadmill_environment = false;
	dmTreadmill* treadmill = NULL;
	if (typeid (*(dmEnvironment::getEnvironment ())) == typeid (dmTreadmill))
	{
		treadmill_environment = true;
		treadmill = dynamic_cast <dmTreadmill*> (dmEnvironment::getEnvironment ());
	}
   ...
   for (unsigned int i = 0; i < m_num_contact_points; i++)
   {
      ...
      {                        // wrt ICS.
		...
		bool off_treadmill;
		if (treadmill_environment)
			ground_elevation = treadmill->getGroundElevation(current_pos, normal, &off_treadmill);
		else
			ground_elevation =
      ...
      if (current_pos[2] > ground_elevation)  // No contact
      {
         ...
         {
				// Then previously in contact
            m_contact_flag[i] = false;
            m_boundary_flag = true;

				if (treadmill_environment)
					treadmill->freePoint (m_point_IDs[i]);
         }
         m_sliding_flag[i] = false;
      ...
      else                      // Contact!
      {
         ...
         if (m_contact_flag[i] == false)
         {
            ...
            m_boundary_flag = true;
				if (treadmill_environment)
					m_point_IDs[i] = treadmill->trackPoint (m_initial_contact_pos[i]);
         }
         ...
         vnC_pos[2] += val.v[5];
			...
			CartesianVector anchor_point;
			if (!treadmill_environment)
				copyCartesianVector (m_initial_contact_pos[i], anchor_point);
			else
				treadmill->getPointLocation (m_point_IDs[i],
					anchor_point);
			...
			for (j = 0; j < 3; j++)
         {
            veC_pos[j] = val.R_ICS[j][0]*vnC_pos[0] +
				...
				peC_pos[j] = current_pos[j] - anchor_point[j];
         }

			if (treadmill_environment && !off_treadmill)
			{
				CartesianVector ground_velocity = {0, 0, 0};
				treadmill->getVelocityDirection (ground_velocity);
				// Scale for the magnitude
				int dof = dmEnvironment::getEnvironment ()->getNumDOFs ();
				Float* q = new Float[dof];
				Float* qd = new Float[dof];
				dmEnvironment::getEnvironment ()->getState (q, qd);
				// The last element in the derivatives is the treadmill velocity
				Float treadmill_velocity = qd[dof - 1];
				delete [] q;
				delete [] qd;
				// Add the ground velocity to the current velocity
				veC_pos[0] -= ground_velocity[0] * treadmill_velocity;
				veC_pos[1] -= ground_velocity[1] * treadmill_velocity;
				veC_pos[2] -= ground_velocity[2] * treadmill_velocity;
			}
         ...
         else
         {
            ...
            if (m_sliding_flag[i])
            {
               ...
               m_initial_contact_pos[i][2] = ground_elevation;
					...
					if (treadmill_environment)
					   treadmill->resetPoint (m_point_IDs[i], m_initial_contact_pos[i]);
