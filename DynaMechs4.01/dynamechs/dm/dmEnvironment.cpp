...
dmEnvironment::dmEnvironment()
      : dmSystem(),
        ...
        m_y_dim(0),
        m_grid_resolution(0),
		m_x_grid_resolution(0), 
		m_y_grid_resolution(0), 
        m_depth(NULL),
        m_terrain_filename(),
...
// Outputs  :
// ---------------------------------------------------------------------------
void dmEnvironment::loadTerrainData(const string &filename)
{
   register int i, j;

   m_terrain_filename = filename;

   ifstream data_ptr(filename.c_str());
   if (!data_ptr)
   {
   ...
   data_ptr >> m_x_dim >> m_y_dim >> m_x_grid_resolution >> m_y_grid_resolution;
...
// ---------------------------------------------------------------------------
// Function : loadTerrainData
// Purpose  : Function to allocate terrain based on passed parameters
// Inputs   : number of rows, number of columns, x and y grid spacing, 2x2 matrix of elevations, 
// Outputs  :
// ---------------------------------------------------------------------------
void dmEnvironment::loadTerrainData(int x_dimension, int y_dimension, 
									Float x_grid_res, Float y_grid_res, Float **depthArray)
{
#ifdef DEBUG
	// Some validation
	if (x_dimension < 2 || y_dimension < 2)
	{
		cerr << "Invalid dimension specified in dmEnvironment::loadTerrainData"
			<< endl;
		exit (0);
	}
	if (!depthArray)
	{
		cerr << "Invalid depth array specified in dmEnvironment::loadTerrainData"
			<< endl;
		exit (0);
	}
	if (depth)
	{
		cerr << "depth array already allocated in dmEnvironment::loadTerrainData"
			<< endl;
		exit (0);
	} 
#endif
	...
	m_x_dim = x_dimension;
	m_y_dim = y_dimension;
	m_x_grid_resolution = x_grid_res;
	m_y_grid_resolution = y_grid_res; 
	// allocate space for and read in depth data;
	m_depth = new Float *[m_x_dim];
...
//----------------------------------------------------------------------
void dmEnvironment::loadTerrainData(int x_dimension, int y_dimension, 
									Float grid_res, Float **depthArray)
{
#ifdef DEBUG
	// Some validation
	if (x_dim < 2 || y_dim < 2)
                                    ...
                                    CartesianVector ground_normal) const
{
   ...
   int xindex = (int) (contact_pos[0]/m_x_grid_resolution);
   int yindex = (int) (contact_pos[1]/m_y_grid_resolution);
   ...
   t = (contact_pos[0] -
        ((float) xindex*m_x_grid_resolution))/m_x_grid_resolution;
   u = (contact_pos[1] -
        ((float) yindex*m_y_grid_resolution))/m_y_grid_resolution;
