                               ...
                               Float joint_vel)
{
   ...
   if (source_voltage != 0.0)
   {
      ...
      if (m_max_brush_drop != 0)
         source_voltage -= sgn(source_voltage)*m_max_brush_drop *
            (1.0 - pow((Float)0.5f, (Float)(fabs(source_voltage)/m_half_drop_value)));
   }

   // add torque developed internally by motor (Kt*i).
   Float torque = external_torque + m_torque_constant*m_gear_ratio*(source_voltage -
                        joint_vel/m_gear_ratio*m_back_EMF_constant)/m_armature_resistance;
   ...
   if (joint_vel == 0.0)
   {
      ...
      if (torque != 0.0)
      {
         if (m_coulomb_friction_constant*m_gear_ratio > fabs(torque))
         {
         ...
         {
            torque -= sgn(torque)*m_coulomb_friction_constant*m_gear_ratio;
   ...
   else
   {
      ...
      torque -= speed_sign*m_coulomb_friction_constant*m_gear_ratio +
                joint_vel*m_viscous_friction_constant*m_gear_ratio;
                // note that gear_ratio divisor of the joint_vel cancel with one of the two
                // gear_ratio multipliers of the viscous_friction_constant
      m_stiction_flag = false;
   }
