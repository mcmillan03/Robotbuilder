 ...
 *****************************************************************************
 *     File: dmLoader_dm21.hpp
 *   Author: Scott McMillan
 *  Created: 12 August 2001
 *  Summary: loader for version 2.1 DynaMechs configuration files
 *****************************************************************************/

#ifndef _DM_LOADER_DM21
#define _DM_LOADER_DM21

#include <dmLoader_dm203.hpp>

/**

Loader for V 2.1 configuration files (deprecated).
...
contents.  This class, supporting files for version 2.1, is provided for
backward compatibility and the comment appears follows:
\begin{verbatim}
# DynaMechs V 2.1 ascii
\end{verbatim}
This format differs from the previous 2.0.3 format by adding the 'Name'
parameter to each block.  These names can be used to find objects in a system
with the dmuFindObject(..) function.

This configuration format begins with a specification of 3D models to represent
various objects indexed in later blocks.  This block appears as
follows (for example):
\begin{verbatim}
Graphics_Models {
...
class DMU_DLL_API dmLoader_dm21 : public dmLoader_dm203
{
public:
   /** Constructor. */
   dmLoader_dm21();
...
private:
   dmLoader_dm21(const dmLoader_dm21 &);              // not implemented
   dmLoader_dm21 & operator=(const dmLoader_dm21 &);  // not implemented
