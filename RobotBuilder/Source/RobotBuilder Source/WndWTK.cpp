...
void CWndWTK::InitializeWTKWindow()
{
  ...
  // Set the foreground drawing function to draw axes
	WTwindow_setfgactions (WTuniverse_getwindows(), 
		CWTKGraphics::WTKFGDrawCallback); // and load a start-up logo displayed once 

  WTuniverse_setrendering(WTRENDER_BEST); //Try best rendering, is the graphics better?
...
void CWndWTK::MyWTKMouseMove (WTsensor* wtSensor)
{
	...
	CWTKView* pWTKView = (CWTKView*) pDoc->GetWTKView ();

	if (flagLeftButton | flagRightButton)
	{
		if (pDoc->GetCameraData ()->GetViewInPanMode ())
			pWTKView->m_wndWTK.m_pCameraControl->UpdatePanView (nWndPosX, nWndPosY, nWndWidth, nWndHeight,
		...
		else
		{
			...
			if (pDoc->GetCameraData ()->GetUseViewportFrameForExamineView ())
				pWTKView->m_wndWTK.m_pCameraControl->UpdateExamineView2 (nWndPosX, nWndPosY, nWndWidth, nWndHeight,
					flagLeftButton, flagRightButton, wtmouseRawData->pos[X], wtmouseRawData->pos[Y]);
			else
				pWTKView->m_wndWTK.m_pCameraControl->UpdateExamineView (nWndPosX, nWndPosY, nWndWidth, nWndHeight,
					flagLeftButton, flagRightButton, wtmouseRawData->pos[X], wtmouseRawData->pos[Y]);
		}
	}
...
void CWndWTK::SetViewpointOrientation(WTq wtqQuat)
{
	...
	CMainFrame *pFrameWnd = (CMainFrame*)pApp->m_pMainWnd;
	CDMViewerDoc* pDoc = (CDMViewerDoc*)pFrameWnd->GetActiveDocument ();
	CdmvArticulationData* pArticulation = (CdmvArticulationData*) pDoc->GetSystemData ();
	WTnode *wtnodeArticulation = pArticulation->GetWtkNode ();

	WTwindow_zoomviewpoint (WTuniverse_getwindows());

	((CWTKView* ) GetParent ())->m_WTKGraphics.UpdateCamera (TRUE);
...
void CWndWTK::ZoomViewpoint(BOOL bZoomIn)
{
	...
	if (bZoomIn)
		...
		dMousePosY = 52;
	if (pDoc->GetCameraData ()->GetViewInPanMode ())
		pWTKView->m_wndWTK.m_pCameraControl->UpdatePanView (0, 0, 100, 100,
			FALSE, TRUE, 50, dMousePosY);
	else
	{
		// Then examine, determine if using viewport frame for update
		if (pDoc->GetCameraData ()->GetUseViewportFrameForExamineView ())
			pWTKView->m_wndWTK.m_pCameraControl->UpdateExamineView2 (0, 0, 100, 100,
			FALSE, TRUE, 50, dMousePosY);
		else
			pWTKView->m_wndWTK.m_pCameraControl->UpdateExamineView (0, 0, 100, 100,
			FALSE, TRUE, 50, dMousePosY);
	}
...
void CWndWTK::GetViewpointOrientation(WTq wtqQuat)
{
	WTviewpoint* wtviewpoint = WTuniverse_getviewpoints ();

	WTviewpoint_getorientation (wtviewpoint, wtqQuat);
}

void CWndWTK::GetViewpointPosition(WTp3 wtp3Pos)
{
	WTviewpoint* wtviewpoint = WTuniverse_getviewpoints();

	WTviewpoint_getposition (wtviewpoint, wtp3Pos);
}

void CWndWTK::SetViewpointPosition(WTp3 wtp3Pos)
{
	WTviewpoint* wtviewpoint = WTuniverse_getviewpoints ();

	WTviewpoint_setposition (wtviewpoint, wtp3Pos);
