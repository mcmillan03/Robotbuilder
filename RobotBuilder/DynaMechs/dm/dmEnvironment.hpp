...
The {\tt getTerrainFilename} function returns a reference to the terrain
filename (string) that was loaded.
...
public:
   ...
   ///
   void loadTerrainData(const string &filename);
   ///
   void loadTerrainData(int x_dimension, int y_dimension, 
   ...
   Float **getTerrainData(int &xdim, int &ydim, Float &x_spacing, Float &y_spacing)
	{
		xdim = m_x_dim; 
		ydim = m_y_dim; 
		x_spacing = m_x_grid_resolution; 
		y_spacing = m_y_grid_resolution; 
		return m_depth; 
	}

   ///
   const string &getTerrainFilename() const {return m_terrain_filename;}
...
private:
   ...
   Float   m_x_grid_resolution;    // distance b/w grid points
   Float   m_y_grid_resolution; 
   Float **m_depth;              // 2D array of depth data.

   string  m_terrain_filename;
